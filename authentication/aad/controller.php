...
namespace Concrete\Package\AzureActiveDirectory\Authentication\Aad;

use \Concrete\Package\AzureActiveDirectory\Authentication\Type\Aad\ServiceFactory;
...
class Controller extends GenericOauth2TypeController
{
    ...
    /** @var \Authentication\Type\Aad\ServiceFactory */
    protected $factory;
    ...
    {
        return (int) $this->config->get('auth.aad.registration.group');
    }
    ...
    {
        return (bool) $this->config->get('auth.aad.registration.enabled', false);
    }
    ...
    public function saveAuthenticationType($args)
    {
        ...
        $this->config->save('auth.aad.name', $passedName);
        $this->config->save('auth.aad.url', $args['url']);
        $this->config->save('auth.aad.appid', $args['apikey']);
        $this->config->save('auth.aad.directoryid', $args['directoryid']);
        $this->config->save('auth.aad.secret', $args['apisecret']);
        $this->config->save('auth.aad.registration.enabled', (bool) $args['registration_enabled']);
        $this->config->save('auth.aad.registration.group', intval($args['registration_group'], 10));
    }
    ...
    public function edit()
    {
        ...
        $this->set('data', $this->config->get('auth.aad', []));
        
        $this->set('redirectUri', $this->urlResolver->resolve(['/ccm/system/authentication/oauth2/aad/callback']));
    ...
    {
        $data = $this->config->get('auth.aad', '');
        $authUrl = $this->urlResolver->resolve(['/ccm/system/authentication/oauth2/aad/attempt_auth']);
        $attachUrl = $this->urlResolver->resolve(['/ccm/system/authentication/oauth2/aad/attempt_attach']);
        $baseUrl = $this->urlResolver->resolve(['/']);
        $path = $baseUrl->getPath();
        $path->remove('index.php');
        $name = trim((string) array_get($data, 'name', t('AAD')));
